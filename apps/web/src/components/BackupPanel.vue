...
async function previewCleanup() {
  ...
  if (r.deleted.length) {
    ...
    panel.result = {
      kind: 'verify',
      files: r.deleted.map((d) => ({
        path: `${d.repoName} ${d.version}`,
        status: 'removed' as const,
        left: { size: d.items.reduce((s, i) => s + i.size, 0) },
      })),
...
function toggleSelect(r: RepoBackupRef) {
  if (isSelected(r)) {
    selected.value = selected.value.filter(
      (s) => !(s.releaseId === r.releaseId && s.repoId === r.repoId),
    )
    return
  }
...
function exportReport() {
  ...
  const rows = r.files
    .map(
      (f) =>
        `| ${STATUS_LABEL[f.status] ?? f.status} | ${f.path} | ${f.left?.size ?? '—'} | ${f.right?.size ?? '—'} | ${f.insertions ?? '—'} | ${f.deletions ?? '—'} |`,
    )
    .join('\n')
  const md = [
    `# 一致性对比报告`,
...
const compareColumns = [
  {
    ...
    render: (row: { status: string }) =>
      h(
        'span',
        { class: `chip code-text ${STATUS_CHIP[row.status] ?? 'chip-dim'}` },
        `${STATUS_PREFIX[row.status] ?? ''} ${STATUS_LABEL[row.status] ?? row.status}`,
      ),
  },
...
function openRestore(ref: RepoBackupRef) {
  ...
  restoreState.kind =
    ref.items.find((i) => ['source-bundle', 'source-archive', 'artifact'].includes(i.kind))?.kind ??
    ''
...
async function doRestore() {
  ...
  try {
    ...
    message.success(
      `已恢复到 ${restoreState.targetDir.trim()}（第 ${r.restores} 次恢复，已入审计）`,
    )
  ...
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div class="glass-panel p-4 rounded-2xl">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-medium flex items-center gap-1.5"
            ><i class="i-carbon-data--base" /> 磁盘占用</span
          >
          <NButton size="tiny" :loading="usageLoading" @click="loadUsage()">刷新</NButton>
        </div>
        <div v-if="usage" class="space-y-1 text-xs">
          <div>
            总计：<span class="font-mono font-medium">{{ formatSize(usage.totalBytes) }}</span> ·
            {{ usage.totalCount }} 份
          </div>
        ...
        <div class="flex gap-2 mt-3">
          ...
          <NButton size="tiny" type="warning" :loading="cleanupLoading" @click="executeCleanup"
            >执行清理</NButton
          >
        </div>
      </div>
      <div class="glass-panel p-4 rounded-2xl">
        <div class="text-sm font-medium mb-2 flex items-center gap-1.5">
          <i class="i-carbon-settings" /> 保留策略（按仓库）
        </div>
        <div class="grid grid-cols-3 gap-2">
          <div>
            <div class="text-xs text-text-3 mb-1">保留份数</div>
            <NInputNumber
              v-model:value="retentionForm.keepLast"
              placeholder="不限"
              :min="1"
              size="small"
              clearable
          ...
          <div>
            ...
            <NInputNumber
              v-model:value="retentionForm.maxBytesMB"
              placeholder="不限"
              :min="1"
              size="small"
              clearable
          ...
          <div>
            ...
            <NInputNumber
              v-model:value="retentionForm.keepDays"
              placeholder="不限"
              :min="1"
              size="small"
              clearable
    ...
    <div v-if="canCompare" class="glass-panel p-3 rounded-xl flex items-center gap-2 flex-wrap">
      <span class="text-xs text-text-2"
        >已选: {{ orderedSelected[0].version }} ↔ {{ orderedSelected[1].version }}</span
      >
      <NButton type="primary" size="small" @click="runCompare('artifact')"
        ><template #icon><i class="i-carbon-compare" /></template>产物对比</NButton
      >
      <NButton size="small" secondary @click="runCompare('source')"
        ><template #icon><i class="i-carbon-git-compare" /></template>源码对比</NButton
      >
      ...
      <div v-else class="space-y-5">
        <section v-for="repo in project.repos" :key="repo.id" class="glass-panel p-5 rounded-2xl">
          <div class="flex items-center gap-2 mb-3">
            ...
            <span v-if="repo.artifactDir" class="code-text text-xs text-text-3"
              >产物目录：{{ repo.artifactDir }}</span
            >
            <span v-else class="code-text text-xs text-text-3">未配置产物目录（仅源码备份）</span>
          </div>
          <div
            v-if="(backupsByRepo[repo.id]?.length ?? 0) === 0"
            class="py-4 text-center text-xs text-text-3"
          >
            暂无备份——发布一次后自动生成
          </div>
          <div v-else class="space-y-2">
            <div
              ...
              :key="r.releaseId"
              class="flex items-center gap-3 px-3.5 py-3 rounded-[var(--bx-radius-md)] border bg-surface transition-[border-color,background-color] duration-fast"
              :class="
                isSelected(r)
            ...
            >
              ...
              <span
                class="w-[34px] h-[34px] flex items-center justify-center rounded-[9px] bg-surface-alt border border-border text-text-3 shrink-0"
                ><i
                  class="text-16px"
                  :class="KIND_ICON[r.items[0]?.kind ?? ''] ?? 'i-carbon-database'"
              /></span>
              <div class="min-w-0 flex-1">
                ...
                <div class="flex items-center gap-2 mt-1 flex-wrap">
                  <span v-for="item in r.items" :key="item.kind" class="chip"
                    >{{ KIND_LABEL[item.kind] ?? item.kind }} · {{ formatSize(item.size) }}</span
                  >
                  <span
                    v-if="r.restores?.length"
                    class="chip chip-brand"
                    :title="`最近恢复：${formatDateTime(r.restores[r.restores.length - 1].at)} → ${r.restores[r.restores.length - 1].targetDir}`"
                  >
              ...
              <div class="flex items-center gap-1 shrink-0">
                <NDropdown
                  trigger="click"
                  :options="
                    r.items.map((i: BackupItem) => ({
                      label: `下载 ${KIND_LABEL[i.kind] ?? i.kind}（${formatSize(i.size)}）`,
                      key: i.kind,
                    }))
                  "
                ...
                >
                  <button
                    class="w-7 h-7 flex items-center justify-center rounded-md text-text-3 hover:bg-surface-hover hover:text-text-1 transition-colors focus-ring"
                    aria-label="下载"
                    title="下载"
                  >
                ...
                <button
                  class="w-7 h-7 flex items-center justify-center rounded-md text-text-3 hover:bg-error-soft hover:text-error transition-colors focus-ring"
                  aria-label="删除"
                  title="删除"
    ...
    >
      <div v-if="panel.loading"><LoadingState text="正在对比…" /></div>
      <template v-else-if="panel.result">
        <div class="flex items-center gap-2 mb-3 flex-wrap">
          ...
          <NButton size="tiny" secondary @click="exportReport"
            ><template #icon><i class="i-carbon-download" /></template>导出校验报告</NButton
          >
        </div>
        <NDataTable
          size="small"
    ...
    <!-- 恢复（M7 收口） -->
    <NModal
      v-model:show="restoreState.open"
      preset="card"
      title="恢复备份到本地目录"
    ...
    >
      <div class="space-y-3">
        <div class="text-xs text-text-3 leading-relaxed">
          目标目录须位于 BX_HOME 白名单内{{
            bxHome ? `（${bxHome} 及子目录）` : ''
          }}，不存在将自动创建； 不触碰业务仓库（零侵入）。恢复成功会写入审计记录。
        </div>
        <div>
          <div class="text-xs text-text-2 mb-1">
            备份：{{ restoreState.ref?.repoName }} {{ restoreState.ref?.version }}
          </div>
          <NRadioGroup v-model:value="restoreState.kind">
            <NSpace>
              <NRadio v-for="it in restoreState.ref?.items ?? []" :key="it.kind" :value="it.kind"
                >{{ KIND_LABEL[it.kind] ?? it.kind }} ({{ formatSize(it.size) }})</NRadio
              >
            </NSpace>
          </NRadioGroup>
        </div>
        <div>
          ...
          <div class="flex gap-2">
            <NInput
              v-model:value="restoreState.targetDir"
              placeholder="BX_HOME 内的绝对路径…"
              class="flex-1"
              autocomplete="off"
              spellcheck="false"
            />
        ...
        <NCheckbox
          v-if="restoreState.kind !== 'source-bundle'"
          v-model:checked="restoreState.overwrite"
        >
        ...
        <div>
          <div class="text-xs text-text-2 mb-1">
            二次确认：输入版本号
            <span class="font-mono text-text-1">{{ restoreState.ref?.version }}</span> 解锁恢复
          </div>
          <NInput
