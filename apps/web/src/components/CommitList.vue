  ...
  <div class="space-y-1">
    ...
    >
      <div class="flex items-center gap-2 min-w-0">
        <i aria-hidden="true" class="text-14px shrink-0" :class="[COMMIT_TYPE_ICONS[c.type], c.breaking ? 'text-error' : 'text-text-3']" />
        ...
        <button
          type="button"
          class="code-text text-text-3 shrink-0 cursor-pointer hover:text-brand-500 transition-colors duration-150 bg-transparent border-none p-0"
          :title="c.fullHash"
          aria-label="复制完整哈希"
          @click="emit('copy', c.fullHash)"
        >{{ c.hash.slice(0, 7) }}</button>
        <span class="text-xs text-text-3 shrink-0 hidden sm:inline">{{ c.author }} · {{ c.date }}</span>
        <button
          v-if="showFiles && c.files.length"
          type="button"
          aria-label="展开/收起文件"
          class="i-carbon-chevron-down text-text-3 cursor-pointer transition-transform duration-150 shrink-0 bg-transparent border-none p-0"
