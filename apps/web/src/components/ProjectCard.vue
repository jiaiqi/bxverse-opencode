...
<template>
  <RouterLink
    ...
    class="p-5 rounded-2xl border border-border bg-surface hover:border-border-strong transition-[border-color,box-shadow,background-color,transform] duration-200 flex flex-col justify-between group shadow-sm hover:shadow-md focus-ring"
  >
    <div>
      <!-- 顶部：项目名称与版本号 -->
    ...
    <div
      class="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs font-mono"
    >
      <div class="text-[11px] text-text-3 truncate max-w-[170px]" :title="project.lastRelease ? `${project.lastRelease.date} 发布 ${project.lastRelease.version}` : ''">
      ...
      <div v-if="project.dirtyRepoCount > 0" class="flex items-center gap-1 text-[10px] text-warning mr-1" :title="`${project.dirtyRepoCount} 个仓库工作区有未提交改动`">
