...
<template>
  ...
  <div class="flex h-screen overflow-hidden bg-bg font-sans text-text-1 gap-3 p-3">
    ...
    <a
      ...
      >跳到主内容</a
    >
    <!-- 左：悬浮玻璃侧栏（品牌 + 项目快捷列表 + 主导航 + 底部工具）—— R34 布局 -->
    <aside
      class="app-rail w-[232px] shrink-0 rounded-2xl border border-border bg-surface flex flex-col overflow-hidden"
    >
      <RouterLink
        to="/"
        class="flex items-center gap-2.5 px-5 pt-5 pb-4 no-underline text-inherit cursor-pointer"
      >
        <span
          class="w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs font-mono shrink-0"
          style="background: var(--bx-text-1); color: var(--bx-bg)"
          >BX</span
        >
        <span class="min-w-0">
          <span class="block font-extrabold tracking-tight text-text-1 text-[15px] leading-tight"
            >BXVERSE</span
          >
          <span class="block text-[10px] text-text-3">版本管理台</span>
        </span>
      </RouterLink>

      <div class="px-4 pb-1 text-[10px] font-semibold tracking-[0.14em] text-text-3">项目</div>
      <div class="px-2 space-y-0.5 max-h-52 overflow-y-auto">
        <button
          v-for="p in projectsStore.items"
          :key="p.id"
          @click="router.push(`/project/${p.id}`)"
          class="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-left text-xs font-semibold transition-colors duration-fast cursor-pointer border-0 focus-ring"
          :class="
        ...
        >
          <span
            class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold font-mono shrink-0"
            :class="
              currentProject?.id === p.id ? 'bg-brand-500 text-white' : 'bg-surface-alt text-text-3'
            "
            >{{ p.name.slice(0, 1) }}</span
          >
          <span class="truncate">{{ p.name }}</span>
          <span class="ml-auto font-mono text-[10px] text-text-3">{{ p.version }}</span>
        </button>
        <button
          @click="showAddProject = true"
          class="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-left text-xs font-semibold text-text-3 hover:text-brand-600 hover:bg-surface-hover transition-colors duration-fast cursor-pointer border-0 focus-ring"
        >
          <i aria-hidden="true" class="i-carbon-add text-14px" /> 新建项目
        </button>
      </div>

      <div class="px-4 pt-4 pb-1 text-[10px] font-semibold tracking-[0.14em] text-text-3">
        工作台
      </div>
      <nav class="flex-1 overflow-y-auto px-2 space-y-0.5" aria-label="主导航">
        <RouterLink
          v-for="nav in navItems"
          :key="nav.id"
          :to="nav.to"
          class="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] font-medium no-underline transition-colors duration-fast cursor-pointer"
          :class="
            route.path === nav.to ||
            (nav.id === 'project' &&
            ...
            (nav.id === 'backups' && route.path.includes('/backups'))
              ? 'bg-brand-soft text-brand-600 font-semibold'
              : 'text-text-2 hover:bg-surface-hover hover:text-text-1'
          "
          :title="nav.label"
        >
      ...
      <div class="border-t border-border px-2 py-2">
        <button
          ...
          class="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] text-text-2 hover:bg-surface-hover hover:text-text-1 transition-colors duration-fast cursor-pointer border-0 focus-ring"
          title="重看新手引导（M5-08）"
        >
          <i aria-hidden="true" class="i-carbon-help text-16px" /> <span>新手引导</span>
        </button>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <!-- 顶栏全局控制台 -->
      <header
        class="app-topbar h-12 rounded-2xl border border-border bg-surface px-4 flex items-center justify-between shrink-0 z-30"
      >
        <!-- 左：项目切换器 -->
      ...
      <!-- 主视口内容区（R34：stone 下 bg-bg 转透明，环境光场透出） -->
      <main id="main-content" class="flex-1 overflow-y-auto min-w-0 bg-bg" tabindex="-1">
        <slot />
      </main>
    </div>
