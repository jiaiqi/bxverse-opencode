...
type DoctorRepoReport = {
  repoId: string; repoName: string; head: string; branch: string;
  lastPublishCommit: string | null; dirty: number; ahead: number; baseAncestor: boolean;
  otherBranches: string[]; buildTagsRecent: string[]; vTagsCount: number; vTagLatest: string | null;
  plainTagsCount: number; plainTagsRecent: string[]; packageJsonLastCommit: string | null;
  state: 'ok' | 'warn' | 'error' | 'checking'; hints: string[];
}
type DoctorProjectReport = { projectId: string; projectName: string; projectVersion: string; repos: DoctorRepoReport[] }
type DoctorReport = { home: string; at: string; counts: { ok: number; warn: number; error: number }; overall: 'ok' | 'warn' | 'error' | 'checking'; projects: DoctorProjectReport[] }
...
const filteredProjects = computed<DoctorProjectReport[]>(() => {
  ...
  return projects.filter((p) => p.projectName.toLowerCase().includes(q) || p.projectId.toLowerCase().includes(q))
})
...
function stateChip(s: DoctorRepoReport['state']): { type: 'success' | 'warning' | 'error'; label: string } {
...
const logLines = ref<Array<{ ts: string; level: string; message: string; fields: Record<string, unknown> }>>([])
...
const migrationPlan = computed(() => {
  ...
  return [
    { from: 'app.json embedded projects[]', to: 'projects/<id>.json 单文件', status: 'done', note: 'core 自动迁移：旧项目自动拆出；缺字段时回退内嵌' },
    { from: '旧项目 lastPublishCommit=null', to: '首次发布前不下发，只走全量收集', status: 'done', note: 'maxCommits=500 兜底' },
    { from: '无', to: 'schemaVersion 字段（计划中）', status: 'pending', note: 'M13 后续：迁移前自动备份 app.json.bak-mig，可回滚' },
  ]
})
...
onMounted(() => {
  timer = setInterval(() => { void loadProcess() }, 30_000)
})
  ...
  <div class="page max-w-6xl space-y-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <section class="glass-panel p-5 rounded-2xl">
        <div class="flex items-center justify-between mb-2">
          ...
          <NTag v-if="doctorReport" :type="doctorReport.overall === 'ok' ? 'success' : doctorReport.overall === 'warn' ? 'warning' : 'error'" :bordered="false" size="small">
            整体 {{ doctorReport.overall }} · ok {{ doctorReport.counts.ok }} / warn {{ doctorReport.counts.warn }} / error {{ doctorReport.counts.error }}
          </NTag>
        </div>
        <p class="text-xs text-text-3 mb-3">核对 app.json 的 lastPublishCommit 与各仓库真实 git 状态，专治「全部显示最新 / 提交流为空」类问题。</p>
        <div class="flex items-center gap-2 flex-wrap mb-3">
          <NInput v-model:value="doctorFilter" placeholder="筛选项目名/ID…" clearable size="small" style="width: 200px" />
          <NButton size="tiny" :loading="doctorLoading" @click="runDoctor">立即全量体检</NButton>
          <NButton v-if="doctorReport" size="tiny" quaternary @click="loadDoctor" :loading="doctorLoading">刷新</NButton>
          <NButton v-if="doctorReport" size="tiny" quaternary @click="exportDoctorReport">导出报告（md）</NButton>
        </div>
        <div v-if="doctorReport" class="space-y-3 max-h-96 overflow-y-auto pr-1">
          <div v-if="filteredProjects.length === 0" class="text-xs text-text-3 text-center py-3">无匹配项目</div>
          <div v-for="p in filteredProjects" :key="p.projectId" class="rounded-lg border border-border bg-surface-alt p-3">
            <div class="flex items-center gap-2 mb-2">
              <i aria-hidden="true" class="i-carbon-catalog text-text-3" />
              <b class="text-text-1 text-sm">{{ p.projectName }}</b>
              <code class="text-xs text-text-3 font-mono">{{ p.projectId }} · v{{ p.projectVersion }}</code>
            </div>
            <div v-if="p.repos.length === 0" class="text-xs text-text-3 pl-5">该项目下无仓库</div>
            <ul v-else class="space-y-1.5 pl-5">
              <li v-for="x in p.repos" :key="x.repoId" class="flex items-start gap-2 text-[12px] font-mono">
                <NTag :type="stateChip(x.state).type" size="tiny" :bordered="false" class="shrink-0">
                  {{ stateChip(x.state).label }}
                </NTag>
                <span class="text-text-1 truncate flex-1" :title="x.repoName">{{ x.repoName }}</span>
                <span class="text-text-3 shrink-0">HEAD {{ x.head }} · base {{ x.lastPublishCommit?.slice(0, 8) ?? '—' }} · ahead {{ x.ahead }} · dirty {{ x.dirty }}</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <section class="glass-panel p-5 rounded-2xl">
        ...
        <p class="text-xs text-text-3 mb-3">未来 schema 升级逐步执行、失败即停、可回滚。当前所有迁移已完成（v1 → v2）。</p>
        <div class="space-y-2 text-xs">
          <div v-for="(m, i) in migrationPlan" :key="i" class="flex items-start gap-2 rounded-md border border-border bg-surface-alt p-2.5">
            <NTag :type="m.status === 'done' ? 'success' : 'default'" :bordered="false" size="tiny" class="shrink-0">
        ...
        <p class="text-[10px] text-text-3 mt-3">BX_HOME 实际位置：<code class="font-mono">{{ processInfo?.home ?? '—' }}</code></p>
      </section>
    </div>
    ...
    <section class="glass-panel p-5 rounded-2xl">
      <div class="flex items-center gap-3 mb-3 flex-wrap">
        ...
        <div class="flex gap-1">
          <button v-for="lv in (['all', 'info', 'warn', 'error'] as const)" :key="lv"
                  @click="logLevel = lv; loadLogs()"
                  class="px-3 py-1 rounded-full text-[10px] mono check-ring border"
                  :class="logLevel === lv ? 'border-brand-500 bg-brand-soft text-brand-500' : 'border-border text-text-3'">
            {{ lv === 'all' ? '全部' : lv.toUpperCase() }}
          </button>
        </div>
        <span class="text-[10px] text-text-3 ml-auto font-mono">
          {{ logFile || '今日日志未生成' }} · 过滤后 {{ logLines.length }} / 累计 {{ logTotal }} · 30 天滚动保留
        </span>
        <NButton size="tiny" :loading="logLoading" @click="loadLogs">刷新</NButton>
      </div>
      <div class="rounded-xl border border-border p-3 max-h-[280px] overflow-y-auto font-mono text-[11px] leading-relaxed" style="background: #0b0d11">
    ...
    <section class="glass-panel p-5 rounded-2xl">
      <div class="flex items-center gap-3 mb-3 flex-wrap">
        ...
        <NTag v-if="processInfo" type="success" :bordered="false" size="small">v{{ processInfo.version }} 已运行 {{ formatUptime(processInfo.uptimeSec) }}</NTag>
        <NTag :bordered="false" size="small">node {{ processInfo?.nodeVersion ?? '—' }} · {{ processInfo?.platform ?? '—' }}</NTag>
        <span class="text-[10px] text-text-3 ml-auto font-mono">进程 RSS {{ processInfo?.memMB ?? 0 }} MB · 启动 {{ processInfo?.startedAt?.slice(0, 19) ?? '—' }}</span>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
        <div>
          <div class="text-text-3 mb-1">安装 / 升级</div>
          <div class="flex gap-1">
            <NInput v-model:value="installCommand" size="small" class="flex-1" :input-props="{ autocomplete: 'off', spellcheck: false }" />
        ...
        <div>
          ...
          <div class="flex gap-1">
            <code class="text-[11px] font-mono text-text-1 px-2 py-1.5 rounded border border-border bg-surface-alt flex-1 truncate" :title="processInfo?.home">
              {{ processInfo?.home ?? '—' }}
            </code>
      ...
      <div class="text-[10px] text-text-3 mt-3 leading-relaxed">
        bxverse {{ processInfo?.version ?? '—' }} ·
        启动于 {{ processInfo?.startedAt?.slice(0, 19).replace('T', ' ') ?? '—' }} ·
        数据目录 {{ processInfo?.home ?? '—' }} ·
        自举版本同构复用：自身更新日志由 bxverse 引擎生成（dogfooding）
      </div>
    </section>
  </div>
</template>
