...
import { git, store, version as coreVersion } from '@bxverse/core'
  ...
  router.patch('/api/projects/:id/repos/:rid', async (ctx: Ctx) => {
    ...
    const repo = await services.withCfg(async (cfg) => {
      ...
      if (body.updatePackageVersion !== undefined) {
        if (typeof body.updatePackageVersion !== 'boolean')
          throw apiError(400, 'VALIDATION', 'updatePackageVersion 必须为布尔')
      ...
      if (body.artifactDir !== undefined) {
        ...
        if (dir.split('/').includes('..'))
          throw apiError(400, 'VALIDATION', 'artifactDir 必须是仓库内的相对目录（禁止 .. 越界）')
        r.artifactDir = dir || undefined
      }
      if (body.writeVersionFile !== undefined) {
        if (typeof body.writeVersionFile !== 'boolean')
          throw apiError(400, 'VALIDATION', 'writeVersionFile 必须为布尔')
      ...
      if (body.versionSource !== undefined) {
        ...
        if (!['derived', 'packageJson'].includes(v))
          throw apiError(400, 'VALIDATION', 'versionSource 必须为 derived/packageJson')
      ...
      if (body.packageManager !== undefined) {
        ...
        if (v === '') r.packageManager = undefined
          ...
          if (!['pnpm', 'npm', 'yarn', 'bun'].includes(v))
            throw apiError(400, 'VALIDATION', 'packageManager 必须为 pnpm/npm/yarn/bun')
      ...
      if (body.buildTimeoutMs !== undefined) {
        ...
        if (!Number.isInteger(n) || n < 1000 || n > 60 * 60 * 1000)
          throw apiError(400, 'VALIDATION', 'buildTimeoutMs 必须为 1000~3600000 的整数毫秒')
        r.buildTimeoutMs = n
      }
      if (body.versionSyncCommit !== undefined) {
        const v = String(body.versionSyncCommit)
        if (!['package', 'none'].includes(v))
          throw apiError(400, 'VALIDATION', 'versionSyncCommit 必须为 package/none')
        r.versionSyncCommit = v as RepoDef['versionSyncCommit']
      }
      // 扩展 R35：仓库初始版本号（首次发布生效；按项目 repoVersionFormat 校验规范化，空串清除）
      if (body.initialVersion !== undefined) {
        const v = String(body.initialVersion).trim()
        if (v === '') {
          r.initialVersion = undefined
        } else {
          try {
            r.initialVersion = coreVersion.normalizeInitialVersion(v, project.repoVersionFormat)
          } catch (e) {
