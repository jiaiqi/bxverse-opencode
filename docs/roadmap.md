...
| 里程碑 / 需求 | 状态 | 完成时间 |
|---|---|---|
| v1.0.0 定稿里程碑（M1-M7, R1-R22 全闭环） | ✅ 完成并打标 Tag `v1.0.0` | 2026-08-17 |
| M8 多项目看板与仓库治理中枢（本地路径/Git克隆双模式接入） | 🚀 v2 计划中 | — |
| M9 分支协同巡检与批量对齐引擎（Branch Drift Guard & Batch Checkout） | 🚀 v2 计划中 | — |
| M10 AI 场景特化模型路由（Commit极速/日志优美/Diff深度推理分流） | 🚀 v2 计划中 | — |
| M11 发布废弃审计与 Tag 纠偏撤回（Release Deprecation & Tag Cleanup） | 🚀 v2 计划中 | — |
| M12 全新三栏全景仪表盘 UI/UX 重构（Dark Space Cockpit SPA） | 🚀 v2 计划中 | — |
---
...
| M8 | 多项目看板与仓库治理 | 项目看板、本地路径/Git克隆双模式接入、产物目录与构建命令配置 | ~6 | 多业务线项目卡片与仓库接入闭环 |
| M9 | 分支协同巡检与对齐引擎 | 分支漂移自动识别、一键批量切主分支、一键批量安全快进拉取 | ~4 | 解决发版前特性分支错发痛点 |
| M10 | AI 场景特化模型路由 | Commit极速生成 / 对外日志优美润色 / Diff深度推理分流指定 | ~5 | 适配不同大模型长处，降低成本提升质量 |
| M11 | 发版废弃审计与Tag纠偏 | 历史发布标为废弃、废弃原因审计、安全撤销本地与远程 Tag | ~4 | 提供发版误操作纠偏与线上紧急回退标记 |
| M12 | 全新三栏全景仪表盘 UI/UX | 沉浸式驾驶舱、深空暗夜碳黑视觉、等宽精密仪表、Ctrl+K命令中心 | ~8 | 实现零跳转高信息密度次世代体验 |
...
| R21 | AI 多供应商体系与凭据安全管理 | 共享契约扩展、core ai 基础封装、server 凭据隔离、设置页供应商管理 UI、LogEditor 适配 |
| R22 | 仓库内 Git 面板与 AI 辅助提交/解读 | core git/ai 扩展、server git API 路由、web GitTab 双栏组件、Conventional Commits 生成与单文件解读 |
