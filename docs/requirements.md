...
| R31 | Version Matrix 矩阵视图（多项目跨工程聚合，0 入侵） | 多项目常共用同一批前端工程（如 `l-pc-front` 在主产品线 / 灰度项目 / 演示项目里都接入），需要「按项目管理工程版本」的可视化矩阵。**核心价值**：①一张表看出每个项目里各工程的当前版本号、上次发布时间、距今天数；②自动高亮「跨项目版本不齐」（某工程在 A 项目是 v1.2.0、在 B 项目是 v1.0.0，矩阵对角线不一致提示对齐）；③展示每个项目下「待发布」工程数与变更提交数。**端到端 0 入侵**——纯聚合展示，不改任何业务仓库，0 写入。①契约层：shared 新增 `VersionMatrix`（`projects[ProjectRow]×cells[repoId→MatrixCell]`；`MatrixCell={app,name,version,lastRelease:{version,date,daysAgo}|null,changed,commits,repoKind}`）// 扩展：R31；②core：`packages/core/src/matrix.ts` 新模块，聚合函数 `buildMatrix(cfg,pollCache,dataStore): Promise<VersionMatrix>`，复用 `services.poll.get` + `runWithPool` 6 仓并行 + `dataStore.listRecords({full:false,limit:1})` 拿 `lastRelease` 摘要；项目行按配置顺序输出，repo 列按「出现最频繁→少→无」聚合排序，列名带该工程出现项目数（如 `l-pc-front · 3 项目`），未接入该工程的项目格用空对象 + 标记 `absent=true`；③server：`GET /api/matrix` 端点返回 `VersionMatrix`（需 X-BX-Token）；④web：路由 `/matrix` + 视图 `VersionMatrix.vue`（`<table>` 矩阵 + sticky 项目列头 + sticky 工程表头 + 单元格 hover 弹窗显示 lastRelease + RouterLink 跳 `RepoDetail` + 搜索框过滤项目/工程 + 「跨项目不齐」行高亮 + 30s 自动刷新）；⑤文档：api.md §路由表 + frontend.md §新页面 + next-development-plan §3 排期 |
| R32 | 升级后回退到历史版本 | 升级上线后发现问题，希望「选择回退到某个历史版本」一键操作。**核心价值**：选 release → 预览影响面（drift / dirty / 兼容性 / 跳过的提交）→ 自动发布回退后新版本 + 标 deprecate 旧版本。**端到端 0 入侵**：仅打标签 + 写 bxverse 数据仓库 + 写 release record（沿用 R24 deprecate 语义），不 amend/commit/force-push 业务仓。①契约层：shared 增 `RollbackPreview`（targetVersion/targetRelease/currentRelease/tagsToDeprecate/repos[]/driftColumnsAffected/nextVersionSuggestion/externalDraft/internalDraft/riskLevel/riskReasons）、`RollbackRequest`（projectId/targetReleaseId/nextVersion/bump/externalContent/internalContent/skipBuild/offline/confirmed/repoIds?）、`RollbackResult`（ok/newReleaseId/deprecatedReleaseIds/failedRepos/deletedTags/warnings）// 扩展：R32；②core：`packages/core/src/rollback.ts` 新模块，函数 `buildRollbackPreview(cfg,targetReleaseId,dataStore,opts)`（取历史 release + 对账 git HEAD + 比对兼容性 + 调 `matrix.buildMatrix` 算 drift + 算 riskLevel）+ `executeRollback(cfg,req,dataStore,services)`（二次确认 → 走发布执行 → 旧 release 链 deprecate → 删多余 tag → 数据仓库 commit 审计）；③server：`GET /api/projects/:id/rollback/preview` + `POST /api/projects/:id/rollback` + `GET /api/projects/:id/releases?includeDeprecated=true` + `GET /api/projects/:id/rollback/diff` 4 端点（app.ts 复用 registerOverviewServices 模式接线）；④web：路由 `/project/:id/rollback`（name `rollback`）+ 视图 `RollbackWizard.vue`（4 步：选 release → 影响面预览 → 版本与日志确认 → 执行）+ ProjectDetail 顶栏「回退到某版本」按钮 + RepoDetail 头部「release 链」下拉 + CommandPalette 命令「rollback-to-version」；⑤门禁：`confirmed===true` 必填 + `riskLevel='block'` 409 拒绝 + dryRun 预览优先 + 业务仓 0 入侵（仅打标签 + 写数据仓库 + 写 release record）；⑥文档：api.md §10.11 + frontend.md §3.8 + core-engine.md §rollback + r32-rollback-guide.md（l-pc-front 去重迁移指南） |
| R33 | 项目与仓库多对多（2026-09-06 立项） | 「项目跟仓库是多对多的：一个项目底下可以有多个仓库，一个仓库也可以在多个项目底下」。两阶段演进——阶段 1（0 契约）：前端「项目管理 + 仓库注册表」聚合视图，按 path 归并同一仓库、展示所属项目 membership，挂载增删复用既有 addRepoByPath/deleteRepo；阶段 2（契约扩展）：shared 增全局仓库注册表与 ProjectDef.repoRefs 引用（仅新增可选字段），registry 成为仓库配置权威源，消除跨项目副本配置漂移 |
| R34 | 玻璃光影视觉体系（UI 3.0，2026-09-06 立项） | 新增第三套主题 themeStyle='stone'：浅色优先、暖石灰中性 + 金色单强调色、玻璃拟态材质（backdrop-filter + 内高光 + 染色投影）、环境光晕层、细颗粒质感、Bento 总览构图；亮/暗双模式等价；以 design/bxverse-final-vision-v5.html 为形态基准；遵守 WIG 约定与 prefers-reduced-motion / prefers-reduced-transparency 降级 |
...
| 2026-08-31 | 新增 R31：Version Matrix 矩阵视图（多项目跨工程聚合，0 入侵）—— 多项目共用同一批前端工程的版本对齐问题。①契约层：shared `VersionMatrix` + `MatrixCell` 类型（仅新增，`// 扩展：R31`）；②core `buildMatrix()` 聚合；③server `GET /api/matrix`；④web `/matrix` 路由 + `VersionMatrix.vue` 视图；详见 requirements.md R31 行 |
| 2026-08-31 | 调研 + 立项 R32 升级后回退到历史版本：基于 l-pc-front 调研（3 大能力缺口：l-pc-front 无回退 / bxverse 只有失败回滚 + 备份恢复 / 用户新需求"选某版本回退"未实现）。①契约层：shared `RollbackPreview`/`RollbackRequest`/`RollbackResult`（仅新增，`// 扩展：R32`）；②core `buildRollbackPreview()` + `executeRollback()`；③server 4 端点；④web `/project/:id/rollback` 路由 + `RollbackWizard.vue` 4 步向导；⑤门禁：confirmed 必填 + riskLevel='block' 409 + dryRun 预览优先 + 业务仓 0 入侵；详见 requirements.md R32 行 + `docs/r32-rollback-guide.md` |
